...
for ii = 1:length(data)
    ...
    % EXERCISE:
    initialMu = 500; %mean(RRTs);
    initialSigma = 5000; % std(RRTs);
    ...
    subplot(3,4,ii); cla reset; hold on;
    later_plotHistogram(RTs, rtBins, 'RT (sec)')

    subplot(3,4,ii+4); cla reset; hold on;
    later_plotHistogram(RRTs, rrtBins, 'RT (sec)')


    subplot(3,4,ii+8); cla reset; hold on;
    y = normpdf(rrtBins, finalFits(1), finalFits(2));
    title(['Gaussian From Returned Fits ' int2str(ii)])
